# @hip4/sdk - Exhaustive Feature Reference
...
| **Peer dependencies**    | None. React bindings are in `@hip4/sdk-react`.                                                                                                                                                                                                                                                    |
...
| ----------------- | -------------------- | -------------------------- |
| `@hip4/sdk`       | `src/index.ts`       | Adapter, types, wallet     |
| `@hip4/sdk/types` | `src/types/index.ts` | Pure type definitions only |
...
Standalone modules (importable from "@hip4/sdk"):
...
| Market types (v1.8)        | `HIP4Market`, `DefaultBinaryMarket`, `LabelledBinaryMarket`, `MultiOutcomeMarket`, `PriceBucketMarket`, `MarketType`, `MarketSide`, `FetchMarketsParams`, `MarketsByType`, `MarketsByQuestion`                                                                                                                                                        |
...
| Core ↔ EVM bridging (v1.8) | `deriveCoreEvmSystemAddress`, `HYPE_CORE_EVM_SYSTEM_ADDRESS`, `MAX_CORE_EVM_TOKEN_INDEX`, `CORE_TO_EVM_GAS_LIMIT`, `estimateCoreToEvmFee`, `medianBaseFeeWei`, `findHypeUsdcSpotPairCoin`, `selectHypeSpotMarkPx`, `HYPE_USDC_SPOT_PAIR_*`                                                                                                            |
| Streams (v1.8)             | `createPriceFeed`, `createPerpPriceFeed`, `processTick`, `candleBoundaryMs`, `intervalToMs`, `PriceFeedSnapshot`, `PerpPriceFeedSnapshot`, `PriceFeedCandle`                                                                                                                                                                                          |
| Orderbook utils (v1.8)     | `computeTradeCost`, `computeEstimatedCost`, `computePotentialReturn`, `TradeCostResult`                                                                                                                                                                                                                                                               |
...
> **v1.8**: `builderAddress` / `builderFee` were added at the adapter level. They can also be passed per order on `placeOrder` to override the adapter-level values.
...
React bindings (context provider + hooks) are available as a separate package: `@hip4/sdk-react`.

```bash
npm install @hip4/sdk-react
```
...
See [@hip4/sdk-react](https://github.com/dennisfurrer/hip4-react) for full documentation.
...
16. **FOK is rejected**: HL's `Ioc` is _Fill-and-Kill_ (allows partial fills), not Fill-or-Kill. As of v1.8 the SDK rejects FOK explicitly so callers don't silently receive partial-fill behavior. Use `FAK` for IoC semantics or `GTC` for resting orders.
...
```tsx
import { createHIP4Adapter } from "@hip4/sdk";
import { PredictionsAdapterProvider } from "@hip4/sdk-react";
...
```tsx
import { useEvents, useEventDetail } from "@hip4/sdk-react";
...
```tsx
import { usePredictionPrice } from "@hip4/sdk-react";
...
```tsx
import { usePredictionBook } from "@hip4/sdk-react";
...
```tsx
import { usePredictionPositions } from "@hip4/sdk-react";
...
```ts
import { createHIP4Adapter } from "@hip4/sdk";
import type { HIP4Signer } from "@hip4/sdk";
...
// Cancel one or more orders (v1.8: takes an array, returns HLCancelResponse)
